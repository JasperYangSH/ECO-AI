sap.ui.define([
    ...
    return Controller.extend("requestui.controller.OfficalRightScreen", {
        ...
        onInit: function () {
            ...
            this.getView().setModel(reqModel, 'req');
        ...
        onActionPressed: function (oEvent) {
            this.requestFragment ??= this.loadFragment({
                name: "requestui.view.NewRequest"
            });

            this.requestFragment.then((oFragment) => oFragment.open());
        },
        onCloseFragment: function (oEvent) {
            this.byId("NewRequest").close();
        },
        onSubmitFragment: function (oEvent) {
            const reqModel = this.getView().getModel('req');
            
            // var sPath = this.getView().getBindingContext().getPath();
            var sConversion = this.getView().getBindingContext().getPath().split("/Conversation(").slice(-1).pop().slice(0,-1);
            const payload = JSON.stringify({
                noticeId: self.crypto.randomUUID(),
                equipment: reqModel.getProperty("/equipment"),
                location: reqModel.getProperty("/location"),
                conversationID: sConversion
            });
        ...
        newReq:function(payload){
            return new Promise((resolve, reject) => {
                $.ajax({
                    // url: this.getBaseURL() + "/odata/v4/chat/getChatRagResponse",
                    url: sessionStorage.getItem("isDeployedVersion")==="true"?this.getBaseURL() + "/odata/v4/maintenance/createReq":"/odata/v4/maintenance/createReq",
